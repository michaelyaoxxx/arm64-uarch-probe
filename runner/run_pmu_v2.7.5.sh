#!/bin/bash
# GB10 PMU Cache Validation  v2.7.5
# Changelog:
...
#                    实测 4MB: force_rounds=0→7.91ns vs force_rounds=5→7.11ns（差0.8ns）
#              修正1: Section 2/4 L3-hit/L3-hit+ 点 force_rounds: 0 → 5
#              修正2: Section 5 X925 hugepage force_rounds: 1 → 5
#                    实测 4MB: f1=5.59ns vs f5=4.94ns（差0.65ns，warm不足）
#                    大size端(12~16MB) f1≈f5（差<0.05ns，大size warm充分）
#              统一原则: 所有 warm 测量点 force_rounds=5，消除该变量
# ============================================================
...
OUTDIR="../data/$(date +%Y%m%d)_v2.7.5/raw"
mkdir -p $OUTDIR
OUTFILE="$OUTDIR/run_$(date +%Y%m%d_%H%M%S).txt"

exec > >(tee "$OUTFILE") 2>&1

echo "# GB10 PMU Cache Validation  v2.7.5"
...
echo "# BUG-07 FIX: 所有 warm 测量点统一 force_rounds=5"
echo "#   消除 meas_rounds 变量对 4K page L3 set conflict 的干扰"
echo "# Topology:"
...
# ----------------------------------------------------------------
# warm 模式（MB 粒度）
# force_rounds 统一由调用方传入，不使用默认值 0
# ----------------------------------------------------------------
run_warm() {
...
# BUG-07: force_rounds 统一为 5
run_warm $CPU 1  "C0-X925 L2-hit"          5 5
run_warm $CPU 2  "C0-X925 L2->L3 boundary" 5 5
run_warm $CPU 4  "C0-X925 L3-hit"          5 5
run_warm $CPU 6  "C0-X925 L3-hit+"         5 5
run_warm $CPU 8  "C0-X925 L3-tail"         5 5
...
for size_kb in 128 256 384 448 480 512 576 640 768 1024; do
    run_warm_kb $CPU $size_kb "C0-A725 L2-scan ${size_kb}KB" 5 5
done

# BUG-07: L3-hit/L3-hit+ force_rounds: 0 → 5（消除 meas_rounds=50 的 set conflict）
run_warm $CPU 4  "C0-A725 L3-hit"   5 5
run_warm $CPU 6  "C0-A725 L3-hit+"  5 5
run_warm $CPU 8  "C0-A725 L3-tail"  5 5
run_warm $CPU 12 "C0-A725 SLC-hit"  5 5
run_warm $CPU 16 "C0-A725 SLC-hit+" 5 5
...
# BUG-07: force_rounds 统一为 5
run_warm $CPU 1  "C1-X925 L2-hit"          5 5
run_warm $CPU 2  "C1-X925 L2->L3 boundary" 5 5
run_warm $CPU 4  "C1-X925 L3-hit"          5 5
run_warm $CPU 8  "C1-X925 L3-hit+"         5 5
run_warm $CPU 12 "C1-X925 L3-hit++"        5 5
run_warm $CPU 16 "C1-X925 L3-tail"         5 5
...
for size_kb in 128 256 384 448 480 512 576 640 768 1024; do
    run_warm_kb $CPU $size_kb "C1-A725 L2-scan ${size_kb}KB" 5 5
done

# BUG-07: L3-hit/L3-hit+ force_rounds: 0 → 5
run_warm $CPU 4  "C1-A725 L3-hit"   5 5
run_warm $CPU 6  "C1-A725 L3-hit+"  5 5
run_warm $CPU 8  "C1-A725 L3-tail"  5 5
run_warm $CPU 12 "C1-A725 SLC-hit"  5 5
run_warm $CPU 16 "C1-A725 SLC-hit+" 5 5
...
# BUG-07: X925 force_rounds: 1 → 5（消除小size端warm不足噪声）
#   实测: C0-X925 4MB f1=5.59ns vs f5=4.94ns（差0.65ns）
#         C0-X925 16MB f1=4.27ns vs f5=4.23ns（差0.04ns，大size无影响）
# ================================================================
echo ""
echo "============================================================"
echo "# Section 5: hugepage L3 boundary scan (4-core comparison)"
echo "#   C0-A725 cpu0 / C0-X925 cpu5 / C1-A725 cpu10 / C1-X925 cpu15"
echo "#   A725: boundary ~8MB (C0直溢SLC, C1有far-slice台阶)"
echo "#   X925: 无边界跳变，单调递减（Slice均匀分布效应）"
echo "#   BUG-07: 所有核 force_rounds 统一为 5"
echo "============================================================"
...
for size_kb in $BOUNDARY_SIZES_KB; do
    run_warm_hugepage_kb 0  $size_kb "C0-A725-HP ${size_kb}KB" 5 5
done

echo ""
echo "  -- Section 5.2: C0-X925 cpu5 hugepage boundary --"
for size_kb in $BOUNDARY_SIZES_KB; do
    run_warm_hugepage_kb 5  $size_kb "C0-X925-HP ${size_kb}KB" 5 5
done

echo ""
echo "  -- Section 5.3: C1-A725 cpu10 hugepage boundary --"
for size_kb in $BOUNDARY_SIZES_KB; do
    run_warm_hugepage_kb 10 $size_kb "C1-A725-HP ${size_kb}KB" 5 5
done

echo ""
echo "  -- Section 5.4: C1-X925 cpu15 hugepage boundary --"
for size_kb in $BOUNDARY_SIZES_KB; do
    run_warm_hugepage_kb 15 $size_kb "C1-X925-HP ${size_kb}KB" 5 5
done
